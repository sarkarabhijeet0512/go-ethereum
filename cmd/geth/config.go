...
func makeConfigNode(ctx *cli.Context) (*node.Node, gethConfig) {
	...
	// TODO: (cyyber) remove setAccountManagerBackends when PQ is fully implemented
	// Node doesn't by default populate account manager backends
	if err := setAccountManagerBackends(stack.Config(), stack.PQAccountManager(), stack.KeyStoreDir()); err != nil {
		utils.Fatalf("Failed to set account manager backends: %v", err)
	}
	// Node doesn't by default populate account manager backends
	if err := setPQAccountManagerBackends(stack); err != nil {
		utils.Fatalf("Failed to set account manager backends: %v", err)
	}
...
func setPQAccountManagerBackends(stack *node.Node) error {
	conf := stack.Config()
	am := stack.PQAccountManager()
